...
void add_country(sqlite3 *db) {
    ...
    snprintf(sql, sizeof(sql),
             "INSERT INTO country (name, capital, language, population_country, square_country, 
currency, head_country) "
...
void list_regions(sqlite3 *db, int country_id) {
    char sql[100];
    snprintf(sql, sizeof(sql), "SELECT name, capital_region FROM region WHERE country_id = %d", 
country_id);
...
void avg_population(sqlite3 *db, int country_id) {
    char sql[100];
    snprintf(sql, sizeof(sql), "SELECT AVG(population_region) FROM region WHERE country_id = 
%d", country_id);
